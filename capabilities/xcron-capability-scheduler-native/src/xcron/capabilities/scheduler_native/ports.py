...
:mod:`xcron.capabilities.scheduler_native.domain`; it never sees a use-case
result from :mod:`xcron.capabilities.scheduler_native.contracts`.
"""
...
from xcron.capabilities.scheduler_native.domain import PlanChange, ProjectPlan, ProjectState
from xcron.capabilities.scheduler_native.layout import RuntimeLayout
from xcron.contracts import NormalizedJob, SchedulerInspection, UnknownSchedulerBackendError
...
class SchedulerRuntimeOptions:
    ...
    def require_layout(self) -> RuntimeLayout:
        if self.layout is None:
            raise RuntimeError("native scheduler operation requires a workspace runtime layout")
        return self.layout
