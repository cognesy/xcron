...
from xcron.capabilities.scheduler_native.ports import DeploymentPlan
from xcron.capabilities.scheduler_native.domain import DeployedJobState, PlanChangeKind, ProjectState
from xcron.contracts import NormalizedJob, ScheduleKind
from xcron.capabilities.scheduler_native.adapters.process import check_output_logged, run_logged_subprocess
from xcron.capabilities.scheduler_native.layout import RuntimeLayout, wrapper_event_log_path, wrapper_log_paths
from xcron.capabilities.scheduler_native.observability import get_logger
from xcron.capabilities.scheduler_native.state_store import save_project_state
from xcron.capabilities.scheduler_native.wrapper import RenderedWrapper, render_wrapper, write_wrapper
...
def render_launchd_job(
    ...
    *,
    ...
    wrapper = render_wrapper(job, desired_hash, layout=layout, state_root=state_root)
...
def apply_launchd_plan(
    ...
    *,
    ...
    for job in deployment.plan.manifest.jobs:
        ...
        runtime_paths = layout.runtime_paths(job, state_root=state_root)
    ...
    save_project_state(state, layout=layout, state_root=state_root)
    LOGGER.info(
    ...
    *,
    ...
    for plist_path in sorted(selected_agents_dir.glob(f"{LABEL_PREFIX}.{project_id}.*.plist")):
        ...
        if wrapper_path is not None:
            stdout_log_path, stderr_log_path = wrapper_log_paths(wrapper_path)
            event_log_path = wrapper_event_log_path(wrapper_path)
        inspections.append(
            LaunchdInspection(
