...
from xcron.capabilities.scheduler_native.adapters.process import run_logged_subprocess
...
from xcron.capabilities.scheduler_native.wrapper import render_wrapper, write_wrapper
...
def render_cron_block(
    ...
    *,
    ...
    for job in manifest_jobs:
        ...
        rendered_wrapper = render_wrapper(job, desired_hash, layout=layout, state_root=state_root)
...
def inspect_cron_project(project_id: str, *, crontab_path: Path | None = None) -> tuple[CronInspection, ...]:
    ...
    for line in lines:
        ...
        stdout_log_path, stderr_log_path = wrapper_log_paths(wrapper)
        event_log_path = wrapper_event_log_path(wrapper)
        inspections.append(
            CronInspection(
