...
def resolve_manifest_path(
    ...
    *,
    ...
    manifest_dir = (
        (project_root / "schedules").resolve()
        if manifest_dir is None
        else Path(manifest_dir).expanduser().resolve()
    )
...
def load_project_manifest(
    ...
    *,
    ...
    if project_path is None:
        raise ManifestLoadError("project path must be explicit for a manifest operation")
    project_root = Path(project_path).expanduser().resolve()
    if not project_root.is_dir():
        raise ManifestLoadError(f"project path is not a directory: {project_root}")
