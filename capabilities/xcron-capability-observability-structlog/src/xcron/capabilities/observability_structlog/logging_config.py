...
LOGGING_PACKAGE = "xcron.capabilities.observability_structlog.resources.logging"
...
def load_logging_config(
    *,
    ...
    if apply_env:
        config = apply_logging_env_overrides(config, environ=environ)
        _validate_logging_config(config)
    return config


def apply_logging_env_overrides(
    ...
    *,
    ...
    level = env.get(LOG_LEVEL_ENV)
    log_format = env.get(LOG_FORMAT_ENV)
