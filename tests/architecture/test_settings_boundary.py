...
CONFIGURATION_MODULES = source_files("src/xcron/configuration")
...
def test_the_configuration_library_has_exactly_one_importer_of_xcfg() -> None:
    ...
    assert importers == {"src/xcron/configuration/loader.py"}
...
def test_configuration_is_a_leaf_that_no_capability_depends_on() -> None:
    ...
    assert CONFIGURATION_MODULES, "xcron.configuration must not be empty"
    ...
    for path in source_files("src/xcron/capabilities"):
        offenders = sorted(
            ...
            if name.startswith("xcron.configuration")
        )
...
def test_the_composition_root_is_the_only_place_settings_are_loaded() -> None:
    loaders = {
        str(path.relative_to(REPOSITORY_ROOT))
        for path in source_files("src/xcron")
        if "xcron.configuration.api" in imported_modules(path)
    }

    assert loaders == {"src/xcron/runtime/composition.py"}
...
def test_the_workspace_module_owns_what_identity_means() -> None:
    ...
    from xcron.capabilities.workspace.api import XCRON_PROJECT_ENV_VAR
    from xcron.configuration.contracts import ENV_SETTINGS
